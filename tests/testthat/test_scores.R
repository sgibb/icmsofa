...
test_that(".horovitz2sofa", {
    expect_equal(sofa:::.horovitz2sofa(c(500, 400, 4:1 * 100 - 1)),
                 as.integer(c(0, 0:4)))
    expect_equal(sofa:::.horovitz2sofa(c(400, 4:1 * 100 - 1), FALSE),
                 as.integer(c(0:2, 2, 2)))
    expect_equal(sofa:::.horovitz2sofa(c(400, 4:1 * 100 - 1),
                                       c(FALSE, TRUE, FALSE, TRUE, FALSE)),
                 as.integer(c(0:3, 2)))
})

test_that(".circulation2sofa", {
    expect_equal(sofa:::.circulation2sofa(
